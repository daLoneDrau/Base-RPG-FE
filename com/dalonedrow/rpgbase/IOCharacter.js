...
var Watchable = require("../util/Watchable.js");
function IOCharacter() {
     ...
     */
    this.adjustAttributeModifier(attr, val) {
        if (attr == null) {
     ...
     * @throws RPGException if an error occurs
     */
    this.ARX_EQUIPMENT_Release(final int id) throws RPGException {
        int i = ProjectConstants.getInstance().getMaxEquipped() - 1;
        for (; i >= 0; i--) {
     ...
     * @param attr the attribute name
     */
    this.clearAttributeModifier(final String attr) {
     ...
     * @throws RPGException if an error occurs
     */
    this.computeFullStats() throws RPGException {
        // clear mods
     ...
     */
    @Override
    this.removeWatcher(final Watcher watcher) {
        int index = -1;
        for (int i = watchers.length - 1; i >= 0; i--) {
     ...
     */
    this.setBaseAttributeScore(final String attr,
            final float val) {
     ...
     * @throws RPGException if the equipment slot was never defined
     */
    this.setEquippedItem(final int slot,
     ...
     * @throws RPGException if the equipment slot was never defined
     */
    this.setEquippedItem(final int slot, final int id)
            throws RPGException {
        if (slot < 0
                || slot >= equippedItems.length) {
            PooledStringBuilder sb =
